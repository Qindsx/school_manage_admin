<template>
  <el-card header="核酸检测异常列表">
    <el-form :inline="true" :model="formData">
      <el-form-item label="姓名">
        <el-input
          v-model="formData.nucleic_name"
          placeholder="输入姓名查询"
        ></el-input>
      </el-form-item>
    ...
    <el-table max-height="550" border stripe :data="listData">
      ...
      <el-table-column label="姓名" prop="nucleic_name"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      CirclePlus,
      listData: [],
      formData: { nucleic_name: '', phone: '', number: '' },
      currentPage: 1,
      pageSize: 5,
      total: 0,
    };
  ...
  methods: {
    ...
    async queryData() {
      let resp = await API.nucleicacidinfo.getListData({
        ...this.formData,
    ...
    async handleExport() {
      ...
      if (res.data) {
        ...
        writeFile(workBook, '核算异常信息报表.xlsx');
    ...
    async handleDelete(row) {
      ...
      try {
        let resp = await API.nucleicacidinfo.deleteById(row.id);
        if (resp.data.status == 'success') {
